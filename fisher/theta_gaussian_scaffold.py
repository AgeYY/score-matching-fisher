...
class ThetaGaussianScaffold:
    ...
    def fit(
        ...
        *,
        ...
        vars_ = np.full((nb, x_dim), vf, dtype=np.float64)
        counts = np.bincount(bin_idx, minlength=nb)
        global_mean = np.mean(x, axis=0)
        global_var = np.maximum(np.var(x, axis=0), vf)
        for b in range(nb):
            idx = np.flatnonzero(bin_idx == b)
            if idx.size > 0:
                means[b] = np.mean(x[idx], axis=0)
                vars_[b] = np.maximum(np.mean((x[idx] - means[b]) ** 2, axis=0), vf)
            else:
                means[b] = global_mean
                vars_[b] = global_var
