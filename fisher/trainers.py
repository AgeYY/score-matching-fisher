...
def _theta_flow_prior_nll_aux_loss(
    *,
    ...
    for k in range(steps):
        ...
        with torch.set_grad_enabled(True):
            theta_req = theta_t.requires_grad_(True)
            velocity = model(theta_req, t_col)
            div = _theta_flow_exact_divergence(
                velocity=velocity,
                theta_t=theta_req,
                create_graph=create_graph,
        ...
        if not enable_grad:
            theta_t = theta_t.detach()
            log_det = log_det.detach()
    log_p = _standard_normal_log_prob(theta_t) + log_det
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        if has_val and val_loader is not None:
            ...
            for tb, xb in val_loader:
                tb = tb.to(device, non_blocking=True)
                xb = xb.to(device, non_blocking=True)
                val_loss = _theta_flow_conditional_nll_aux_loss(
                    model=model,
                    theta_target=tb,
                    x_cond=xb,
                    n_steps=int(ode_steps),
        ...
        if epoch == 1 or epoch % max(1, int(log_every)) == 0 or epoch == int(epochs):
            if has_val:
                print(
                    f"[theta_flow_nll_ft {epoch:4d}/{int(epochs)}] train_nll={mean_train_loss:.6f} "
                    f"val_nll={mean_val_loss:.6f} val_smooth={val_monitor_losses[-1]:.6f} "
                    f"best_smooth={best_val_loss:.6f} best_epoch={best_epoch}"
                )
            else:
                print(f"[theta_flow_nll_ft {epoch:4d}/{int(epochs)}] train_nll={mean_train_loss:.6f}")

        if has_val and patience_counter >= int(early_stopping_patience):
            stopped_early = True
            stopped_epoch = epoch
            print(
                f"[theta_flow_nll_ft early-stop] epoch={epoch} best_epoch={best_epoch} "
...
def train_prior_theta_flow_likelihood_finetune(
    model: PriorThetaFlowVelocity
    | PriorThetaFlowVelocityFiLMPerLayer
    | PriorThetaFlowVelocityThetaFourierMLP,
    theta_train: np.ndarray,
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        for (tb,) in loader:
            tb = tb.to(device, non_blocking=True)
            loss = _theta_flow_prior_nll_aux_loss(
                model=model,
                theta_target=tb,
                n_steps=int(ode_steps),
                enable_grad=True,
        ...
        if has_val and val_loader is not None:
            ...
            for (tb,) in val_loader:
                tb = tb.to(device, non_blocking=True)
                val_loss = _theta_flow_prior_nll_aux_loss(
                    model=model,
                    theta_target=tb,
                    n_steps=int(ode_steps),
                    enable_grad=False,
        ...
        if epoch == 1 or epoch % max(1, int(log_every)) == 0 or epoch == int(epochs):
            if has_val:
                print(
                    f"[prior_theta_flow_nll_ft {epoch:4d}/{int(epochs)}] train_nll={mean_train_loss:.6f} "
                    f"val_nll={mean_val_loss:.6f} val_smooth={val_monitor_losses[-1]:.6f} "
                    f"best_smooth={best_val_loss:.6f} best_epoch={best_epoch}"
                )
            else:
                print(f"[prior_theta_flow_nll_ft {epoch:4d}/{int(epochs)}] train_nll={mean_train_loss:.6f}")

        if has_val and patience_counter >= int(early_stopping_patience):
            stopped_early = True
            stopped_epoch = epoch
            print(
                f"[prior_theta_flow_nll_ft early-stop] epoch={epoch} best_epoch={best_epoch} "
                f"best_smooth={best_val_loss:.6f} patience={int(early_stopping_patience)}"
            )
            break
    ...
    if has_val and restore_best and best_state is not None:
        model.load_state_dict(best_state)
        print(f"[prior_theta_flow_nll_ft restore-best] restored epoch={best_epoch} val_smooth={best_val_loss:.6f}")
