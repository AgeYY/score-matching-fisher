...
from fisher.trainers import (
    ...
    train_conditional_theta_flow_discrete_scaffold_nll,
...
def normalize_theta_field_method(method: str) -> str:
    ...
    raise ValueError(
        ...
        "'theta_flow_discrete_scaffold_q0','theta_path_integral','x_flow','ctsm_v'}."
    )
...
def _save_h_matrix_dsm_artifacts(
    ...
    if bool(getattr(args, "h_save_intermediates", False)):
        ...
        if h_result.theta_flow_log_base_matrix is not None:
            h_payload["theta_flow_log_base_matrix"] = h_result.theta_flow_log_base_matrix
...
def validate_estimation_args(args: Any) -> None:
    ...
    if str(_tfm_val).strip().lower() == "theta_flow_discrete_scaffold_q0":
        if _flow_epochs != 0:
            raise ValueError(
                "theta_flow_discrete_scaffold_q0 requires --flow-epochs 0 (no FM velocity training; H uses q0 only)."
            )
    elif str(_tfm_val).strip().lower() == "theta_flow_discrete_scaffold_nll":
        if _flow_epochs != 0:
            raise ValueError(
                "theta_flow_discrete_scaffold_nll requires --flow-epochs 0 (NLL-only; no FM velocity training)."
            )
        if ft_epochs <= 0:
            raise ValueError(
                "theta_flow_discrete_scaffold_nll requires --flow-likelihood-finetune-epochs > 0 "
                "(posterior flow is trained by scaffold NLL only)."
            )
    elif _flow_epochs == 0:
        if _tfm_val != "theta_flow":
            raise ValueError(
                ...
                "or theta_flow_discrete_scaffold_q0 (q0-only H; use --flow-epochs 0) "
                "or theta_flow_discrete_scaffold_nll (NLL-only; use --flow-epochs 0)."
            )
        if ft_epochs <= 0:
            raise ValueError(
    ...
    if ft_epochs > 0 and _tfm_val not in ("theta_flow", "theta_flow_discrete_scaffold_nll"):
        raise ValueError(
            "--flow-likelihood-finetune-epochs is only supported with --theta-field-method theta_flow "
            "or theta_flow_discrete_scaffold_nll."
        )
    ...
    if _tfm_val in (
        ...
        getattr(args, "flow_endpoint_loss_weight", 0.0)
    ) > 0.0:
        raise ValueError(
    ...
    if bool(getattr(args, "skip_shared_fisher_gt_compare", False)):
        ...
        if not bool(getattr(args, "prior_enable", True)) and _tfm_val not in (
            "x_flow",
            "theta_flow_discrete_scaffold",
            "theta_flow_discrete_scaffold_nll",
            "theta_flow_discrete_scaffold_q0",
...
def _save_dsm_score_prior_training_losses_npz(
    ...
    *,
    ...
    theta_gaussian_scaffold_bin_n_bins: int = 0,
    theta_gaussian_scaffold_grid_size: int = 0,
    theta_gaussian_scaffold_n_components: int = 0,
    theta_gaussian_scaffold_em_steps: int = 0,
    theta_gaussian_scaffold_variance_floor: float = float("nan"),
    theta_gaussian_scaffold_min_branch_mass: float = float("nan"),
    theta_gaussian_scaffold_source_eps: float = float("nan"),
) -> str:
    ...
    np.savez_compressed(
        ...
        theta_gaussian_scaffold_bin_n_bins=np.int64(theta_gaussian_scaffold_bin_n_bins),
        theta_gaussian_scaffold_grid_size=np.int64(theta_gaussian_scaffold_grid_size),
        theta_gaussian_scaffold_n_components=np.int64(theta_gaussian_scaffold_n_components),
        theta_gaussian_scaffold_em_steps=np.int64(theta_gaussian_scaffold_em_steps),
        theta_gaussian_scaffold_variance_floor=np.float64(theta_gaussian_scaffold_variance_floor),
        theta_gaussian_scaffold_min_branch_mass=np.float64(theta_gaussian_scaffold_min_branch_mass),
        theta_gaussian_scaffold_source_eps=np.float64(theta_gaussian_scaffold_source_eps),
    )
    return path
...
def run_shared_fisher_estimation(
    ...
    *,
    ...
    if theta_field_method in (
        ...
        theta_flow_uniform_prior = theta_field_method in (
            "theta_flow_discrete_scaffold",
            "theta_flow_discrete_scaffold_nll",
            "theta_flow_discrete_scaffold_q0",
        )
        nll_only_discrete_scaffold = theta_field_method == "theta_flow_discrete_scaffold_nll"
        fm_enabled = theta_field_method not in (
            "theta_flow_discrete_scaffold_nll",
            "theta_flow_discrete_scaffold_q0",
        )
        post_model_enabled = theta_field_method != "theta_flow_discrete_scaffold_q0"
        ...
        if not bool(getattr(args, "prior_enable", True)):
            ...
            if theta_field_method == "theta_flow_gaussian_scaffold":
                raise ValueError("theta_flow_gaussian_scaffold currently requires prior model enabled.")
        ...
        if theta_field_method == "theta_flow_gaussian_scaffold":
            theta_gaussian_scaffold = ThetaGaussianScaffold.fit(
                ...
                grid_size=int(getattr(args, "theta_gaussian_scaffold_grid_size", 512)),
                n_components=int(getattr(args, "theta_gaussian_scaffold_n_components", 3)),
                em_steps=int(getattr(args, "theta_gaussian_scaffold_em_steps", 20)),
                variance_floor=float(getattr(args, "theta_gaussian_scaffold_variance_floor", 1e-6)),
                min_branch_mass=float(getattr(args, "theta_gaussian_scaffold_min_branch_mass", 1e-4)),
                source_eps=float(getattr(args, "theta_gaussian_scaffold_source_eps", 1e-6)),
            ...
            scaffold_path = os.path.join(args.output_dir, "theta_gaussian_scaffold.npz")
            np.savez_compressed(scaffold_path, **theta_gaussian_scaffold.to_npz_payload())
            print(
                "[theta_gaussian_scaffold] "
            ...
            print(
                ...
                + (
                    " mode=q0_only_h_no_fm"
                    if theta_field_method == "theta_flow_discrete_scaffold_q0"
                    else (
                        " mode=nll_only_no_fm"
                        if theta_field_method == "theta_flow_discrete_scaffold_nll"
                        else ""
                    )
                ),
                flush=True,
        ...
        if post_model_enabled:
            ...
            post_ckpt_hparams = {}
            print(
                "[theta_flow_discrete_scaffold_q0] skipping FM posterior velocity network (--flow-epochs 0; H uses q0 only).",
                flush=True,
            )
        if fm_enabled:
            ...
            post_train_out = {
                ...
                "param_norm_final": float("nan"),
                "n_clipped_steps": 0,
                "n_total_steps": 0,
                "lr_last": float("nan"),
            }
            post_train_losses = np.asarray([], dtype=np.float64)
            post_val_losses = np.asarray([], dtype=np.float64)
            post_val_monitor_losses = np.asarray([], dtype=np.float64)
            post_best_epoch = 0
            post_stopped_epoch = 0
            post_stopped_early = False
            post_best_val_loss = float("nan")
        ...
        if nll_only_discrete_scaffold:
            if theta_discrete_scaffold is None:
                raise RuntimeError("theta_flow_discrete_scaffold_nll requires a fitted ThetaDiscreteScaffold.")
            print(
                "[theta_flow_discrete_scaffold_nll] posterior NLL training enabled: "
            ...
            post_train_out = train_conditional_theta_flow_discrete_scaffold_nll(
                post_model,
                scaffold=theta_discrete_scaffold,
                theta_train=theta_score_fit,
                x_train=x_score_fit,
                epochs=ft_epochs,
                batch_size=ft_batch,
        ...
        if not theta_flow_skip_prior:
            ...
            if theta_flow_uniform_prior:
                _m = f"[{theta_field_method}]"
                print(
                    f"{_m} uniform prior constant: skipping prior flow construction, "
        ...
        if post_model_enabled:
            ...
            post_ckpt_path = ""
            print(
                "[theta_flow_ckpt] posterior checkpoint skipped (theta_flow_discrete_scaffold_q0: no FM model).",
                flush=True,
            )
        if not theta_flow_skip_prior:
        ...
        if h_result is not None:
            ...
            elif theta_field_method == "theta_flow_gaussian_scaffold":
                print(
                    "[summary] theta_flow_gaussian_scaffold mode completed (H-matrix only path; "
            ...
            elif theta_field_method == "theta_flow_discrete_scaffold_nll":
                print(
                    "[summary] theta_flow_discrete_scaffold_nll mode completed (H-matrix only path; "
                    "posterior theta-flow was trained by discrete-q0 scaffold NLL only; "
                    "H uses the same q0-base ODE likelihood and a uniform constant prior)."
                )
            elif theta_field_method == "theta_flow_discrete_scaffold_q0":
                print(
                    "[summary] theta_flow_discrete_scaffold_q0 mode completed (H-matrix only path; "
                    "no FM training; H uses discrete q0(theta|x) log-prob directly (no posterior ODE); "
                    "prior is a uniform constant)."
                )
            else:
                print(
            ...
            tnpz = _save_dsm_score_prior_training_losses_npz(
                ...
                theta_gaussian_scaffold_grid_size=(
                    int(getattr(args, "theta_gaussian_scaffold_grid_size", 512))
                    if theta_field_method == "theta_flow_gaussian_scaffold"
                    else 0
                ),
                theta_gaussian_scaffold_n_components=(
                    int(getattr(args, "theta_gaussian_scaffold_n_components", 3))
                    if theta_field_method == "theta_flow_gaussian_scaffold"
                    else 0
                ),
                theta_gaussian_scaffold_em_steps=(
                    int(getattr(args, "theta_gaussian_scaffold_em_steps", 20))
                    if theta_field_method == "theta_flow_gaussian_scaffold"
                    else 0
                ),
                ...
                theta_gaussian_scaffold_min_branch_mass=(
                    float(getattr(args, "theta_gaussian_scaffold_min_branch_mass", 1e-4))
                    if theta_field_method == "theta_flow_gaussian_scaffold"
                    else float("nan")
                ),
