...
def train_binary_ctsm_v_model(
    *,
    ...
    if x0_fit_np.ndim != 2 or x1_fit_np.ndim != 2 or x0_fit_np.shape[1] != x1_fit_np.shape[1]:
        raise ValueError("ctsm_v_binary training expects x0_train/x1_train shape (N,d) with matching d.")
    if x0_fit_np.shape[0] < 1 or x1_fit_np.shape[0] < 1:
        raise ValueError("ctsm_v_binary training requires at least one sample from each class.")
    ...
    if x0_val is not None and x1_val is not None:
        ...
        if x0_val_np.ndim != 2 or x1_val_np.ndim != 2 or x0_val_np.shape[1] != x0_fit_np.shape[1]:
            raise ValueError("ctsm_v_binary validation expects x0_val/x1_val shape (N,d) matching training d.")
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        if not torch.isfinite(loss):
            has_nonfinite = True
            raise RuntimeError(f"Non-finite ctsm_v_binary train loss at epoch {epoch}: {float(loss.detach().cpu())!r}")
        opt.zero_grad(set_to_none=True)
        loss.backward()
            ...
            with torch.no_grad():
                for _ in range(max(1, int(val_batches_per_epoch))):
                    ...
                    if not torch.isfinite(lv):
                        has_nonfinite = True
                        raise RuntimeError(
                            f"Non-finite ctsm_v_binary validation loss at epoch {epoch}: {float(lv.detach().cpu())!r}"
                        )
                    v_losses.append(float(lv.detach().cpu().item()))
            val_loss = float(np.mean(v_losses))
...
def estimate_binary_ctsm_v_log_ratio(
    ...
    *,
    ...
    if x_np.ndim != 2:
        raise ValueError("ctsm_v_binary log-ratio estimation expects x shape (N,d).")
