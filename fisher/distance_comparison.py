...
class FlowComparisonConfig:
    ...
    likelihood_finetune_epochs: int = TRAINING_MAX_EPOCHS
    ...
    likelihood_finetune_patience: int = TRAINING_EARLY_STOPPING_PATIENCE
    ...
    likelihood_finetune_divergence_estimator: str = "exact"
    likelihood_finetune_hutchinson_probes: int = 1
...
def train_and_estimate_flow_variants(
    *,
    ...
    fine_meta = finetune_flow_skl_cnf_likelihood(
        ...
        divergence_estimator=str(config.likelihood_finetune_divergence_estimator),
        hutchinson_probes=int(config.likelihood_finetune_hutchinson_probes),
        log_every=max(1, int(config.log_every)),
    )
    fine_train_meta = {
        **dict(train_meta),
