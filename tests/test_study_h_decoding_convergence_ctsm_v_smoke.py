...
class TestStudyHDecodingConvergenceCtsmVSmoke(unittest.TestCase):
    def test_ctsm_v_sweep_smoke(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            z = np.load(out_dir / "h_decoding_convergence_results.npz", allow_pickle=True)
            self.assertIn("binned_gaussian_h_binned_columns", z.files)
            self.assertIn("binned_gaussian_corr_h_binned_vs_gt_mc", z.files)
            self.assertEqual(tuple(z["binned_gaussian_h_binned_columns"].shape), (2, n_bins, n_bins))
            self.assertTrue(np.isfinite(z["binned_gaussian_h_binned_columns"]).all())
