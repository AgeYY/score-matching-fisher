...
description: >-
  Score-matching-fisher: when running bin/study_h_decoding_twofig.py with linear_x_flow or
  linear_x_flow_nonlinear_pca on cosinebench or linearbench, use --lxf-early-patience 1000 and
...
When the user asks to run **`linear_x_flow`** and/or **`linear_x_flow_nonlinear_pca`** via **`bin/study_h_decoding_twofig.py`** on **`/cosinebench`** or **`/linearbench`**, apply this fixed hyperparameter bundle unless they explicitly override it.

## Locked choices

| Setting | Value |
|--------|--------|
| Early stopping patience | **`1000`** → pass **`--lxf-early-patience 1000`** (used for the linear X-flow train step **and** the `linear_x_flow_nonlinear_pca` second stage). |
...
**Linearbench** — both methods in one run:
...
2. If **`--lxf-early-patience`** is omitted, the convergence CLI default is **1000** after repo alignment; still pass **`1000`** when following this skill so runs are explicit and reproducible.
