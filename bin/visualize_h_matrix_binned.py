...
def theta_for_h_matrix_alignment(bundle: SharedDatasetBundle, full_args: SimpleNamespace) -> np.ndarray:
    ...
    if tfm in (
        ...
        if not out_dir:
            raise ValueError(
                "theta_flow_discrete_scaffold, theta_flow_discrete_scaffold_nll, or "
                "theta_flow_discrete_scaffold_q0 requires full_args.output_dir for theta_discrete_scaffold.npz"
            )
