#!/usr/bin/env python3
"""Quick visual check for affine geometric-base flow on noisy half-circle data."""
...
def resolve_output_paths(output_dir: Path | None) -> dict[str, Path]:
    out_dir = Path(DATA_DIR) / "geometric_base_half_circle_fit_check" if output_dir is None else Path(output_dir)
    out_dir = out_dir.expanduser().resolve()
    return {
        "output_dir": out_dir,
        "png": out_dir / "geometric_base_half_circle_fit_check.png",
        "svg": out_dir / "geometric_base_half_circle_fit_check.svg",
        "summary": out_dir / "geometric_base_half_circle_fit_check_summary.json",
    }
...
def _make_noisy_half_circle_data(args: argparse.Namespace, condition_values: np.ndarray) -> dict[str, Any]:
    ...
    centers = [
        (float(args.left_center_x), float(args.left_center_y)),
        (float(args.right_center_x), float(args.right_center_y)),
    ]
    arcs = ["upper", "lower"]
    ...
    datasets: list[NoisyHalfCircleBoundaryDataset] = []
    ...
    *,
    ...
    for idx, condition_value in enumerate(condition_values[:, 0]):
        ...
        ax.plot(
            ...
            linewidth=1.2,
            linestyle=":",
            label=f"target half-circle {idx + 1}",
        )
        curve = np.asarray(fitted_curves[idx], dtype=np.float64)
        ax.plot(curve[:, 0], curve[:, 1], color=color, linewidth=2.4, label=f"fitted half-circle {idx + 1}")
    ...
    ax.plot(base_curve[:, 0], base_curve[:, 1], color="#2f2f2f", linewidth=2.0, linestyle="--", label="base half-circle")
    if skl_value is not None:
        ax.text(0.02, 0.98, f"curve SKL = {skl_value:.4g}", transform=ax.transAxes, va="top", ha="left", fontsize=13)
    ax.set_aspect("equal", adjustable="box")
    ax.set_xlabel("x1")
    ax.set_ylabel("x2")
    ax.set_title("Geometric-base affine flow half-circle fit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    data = _make_noisy_half_circle_data(args, condition_values)
    ...
    clean_base = HalfCircleBase(center=(0.0, 0.0), radius=float(args.base_radius))
    base = clean_base
    ...
    target_curves = [ds.boundary(n_points=int(args.curve_points)) for ds in data["datasets"]]
    ...
    training_parameters = {
        ...
        "train_frac": float(args.train_frac),
        "val_frac": float(args.val_frac),
        "max_test_plot_per_condition": int(args.max_test_plot_per_condition),
        "condition_values": condition_values.reshape(-1),
        "theta_encoding": "one_hot",
        "condition_eval": condition_eval,
        "radius": float(args.radius),
        "base_radius": float(args.base_radius),
        "base_distribution": "noisy_half_circle" if base_noise_sigma > 0.0 else "clean_half_circle",
