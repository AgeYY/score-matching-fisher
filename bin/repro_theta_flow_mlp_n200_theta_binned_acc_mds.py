...
This variant runs theta-flow with per-n theta embedding built as:
...
def _default_output_dir(
    ...
    *,
    ...
    tail = f"repro_theta_flow_mlp_n{int(n_sweep)}_{dataset_family}_xdim{x_dim}_obsnoise0p5_{th}_thetabin{_THETA_NUM_BINS}_accmds"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "If omitted, uses data/repro_theta_flow_mlp_n{n}_{dataset_family}_xdim{d}_obsnoise0p5_th{lo}_{hi}_thetabin10_accmds "
    ...
    p.add_argument(
        ...
        type=int,
        default=5,
        help="Embedding dimension for theta-flow acc->H->MDS state (default: 5).",
    )
...
def main() -> None:
    ...
    for tag, results_csv, n_row, corr_h, corr_clf in results:
        ...
        if tag == "theta_flow":
            hm_png = (results_csv.parent / f"h_theta_flow_acc_mds_n_{int(n_row):06d}.png")
            hm_svg = hm_png.with_suffix(".svg")
            hm_csv = (results_csv.parent / f"h_theta_flow_acc_mds_n_{int(n_row):06d}.csv")
            hm_npz = (results_csv.parent / f"h_theta_flow_acc_mds_n_{int(n_row):06d}.npz")
            if hm_png.is_file():
                print(f"[repro][{tag}] final_h_matrix_png={hm_png}", flush=True)
            if hm_svg.is_file():
                print(f"[repro][{tag}] final_h_matrix_svg={hm_svg}", flush=True)
            if hm_csv.is_file():
                print(f"[repro][{tag}] final_h_matrix_csv={hm_csv}", flush=True)
            if hm_npz.is_file():
                print(f"[repro][{tag}] final_h_matrix_npz={hm_npz}", flush=True)
        print(
